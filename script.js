...
async function getSupportTicketTaxonomy(webUrl) {
  const fallback = { issue_id: 1, category_id: 1, sub_category_id: 1 };
  if (!webUrl) return fallback;

  try {
    const taxonomyPath = "/api/v1/tickets/handler/outlet/ticket-taxonomy/";
    const query = new URLSearchParams({ web_url: webUrl }).toString();
    const response = await fetch(`${getSupportApiUrl(taxonomyPath)}?${query}`, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });

    const { ok, data } = await parseSupportApiResponse(response);
    if (!ok || data?.status !== "success") return fallback;

    const ticketTaxonomy = data?.data?.ticket_taxonomy;
    if (!Array.isArray(ticketTaxonomy) || ticketTaxonomy.length === 0) return fallback;

    const firstIssue = ticketTaxonomy[0] || {};
    const firstCategory = (firstIssue.categories || [])[0] || {};
    const firstSubCategory = (firstCategory.sub_categories || [])[0] || {};

    return {
      issue_id: Number(firstIssue.issue_id) || fallback.issue_id,
      category_id: Number(firstCategory.category_id) || fallback.category_id,
      sub_category_id: Number(firstSubCategory.sub_category_id) || fallback.sub_category_id,
    };
  } catch (error) {
    console.warn("⚠️ Failed to fetch ticket taxonomy, using fallback ids.", error);
    return fallback;
  }
}
...
async function createSupportTicket() {
  ...
  const taxonomyIds = await getSupportTicketTaxonomy(web_url);
  const normalizedPriority = priority === "urgent" ? "critical" : priority;
  const relatedOrderValue = document.getElementById("ticketOrderInput")?.value?.trim();

  const tags = [];
  if (relatedOrderValue) tags.push(`order:${relatedOrderValue}`);
  ...
  const payload = {
    ...
    email,
    raised_by: "customer",
    raised_by_id: Number(customer_id) || 0,
    customer_details: {
    ...
    issue_id: taxonomyIds.issue_id,
    category_id: taxonomyIds.category_id,
    sub_category_id: taxonomyIds.sub_category_id,
  };
  ...
  try {
    ...
    const response = await fetch(getSupportApiUrl(getSupportTicketApi().ticketsPath), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
